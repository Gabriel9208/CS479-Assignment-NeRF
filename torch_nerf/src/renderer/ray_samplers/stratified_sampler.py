...
class StratifiedSampler(RaySamplerBase):
    ...
    def sample_along_rays_uniform(
        ...
        t_bin = self.create_t_bins(num_sample + 1, device)
        t_bin = self.map_t_to_euclidean(t_bin, near, far).unsqueeze(0)
        t_samples = (t_bin[:, :-1] + torch.rand(num_ray, num_sample, device=device) * (t_bin[:, 1:] - t_bin[:, :-1]))
